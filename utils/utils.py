...
def draw_label(
    ...
    for bbox, obj_conf, cls_conf, cls_pred, id in zip(bboxs, obj_confs, cls_confs, cls_preds, ids):
        ...
        if id is not None:
            put_str = 'car type:{}'.format(cls_pred) + ' score:{0}'.format(str(cls_conf)[:4]) + 'target ID:{0}'.format(int(id))
        else:
            put_str = 'car type:{}'.format(cls_pred) + ' score:{0}'.format(str(cls_conf)[:4])
